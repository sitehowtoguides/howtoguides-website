...
async function updateGuideDataOnGitHub(guideId, finalUpdateText) {
  if (!GITHUB_OWNER || !GITHUB_REPO || !GITHUB_PAT) {
    throw new Error(
      "Missing GitHub configuration environment variables (OWNER, REPO, PAT)."
    );
  }
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        trimmedLine.startsWith(`id: '${guideId}'`) ||
        trimmedLine.startsWith(`id: "${guideId}"`)
      ) {
        guideStartIndex = i;
        break;
    ...
    if (guideStartIndex === -1) {
      throw new Error(
        `Could not find line starting with id: '${guideId}' in guideData.js`
      );
    }
    ...
    lines[
      updateNotesLineIndex
    ] = `${indentation}updateNotes: ${quoteChar}${updatedNotesContent}${quoteChar}${endsWithComma ? "," : ""}`;
...
export default async function handler(req, res) {
  ...
  if (!id || !action) {
    return res
      .status(400)
      .json({ message: "Missing required fields: id, action" });
  }
  if (action === "approve" && !finalUpdateText) {
    return res
      .status(400)
      .json({ message: "Missing finalUpdateText for approval" });
  }
